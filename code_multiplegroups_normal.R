...
for(id_setting in 1:length(settings_n_k)){
  ...
  for(id_push in 1:2){
    ...
    for(id_diff in 1:l){
      ...
      for(sim_id in 1:num_sim){  
        ...
        for(id_group in 1:length(setting)){
          ...
          if(id_push == 1){
            if(id_group == 2){
              sample <- rnorm(mean = 0 + diff.mean[id_diff], sd = 1, n = n_k)
            } else{
              sample <- rnorm(mean = 0, sd = 1, n = n_k)
            }
          } else if (id_push == 2){
            if(id_group == 1){
              sample <- rnorm(mean = 0, sd = 1, n = n_k)
            } else if(id_group == 2){
              sample <- rnorm(mean = 0.5 + diff.mean[id_diff], sd = 1, n = n_k)
            } else{
              sample <- rnorm(mean = 0.5, sd = 1, n = n_k)
            }
          }
    ...
    picture_name.omnibus <- paste("Omnibus test ",print_setting(setting)," ",
                                  ifelse(push_setting == 1, "(push one group away)",
                                         "(push all away + one more)"), ".png",sep = "")
