...
#second scenario is the steepest
...
for(id_setting in 1:length(settings_n_k)){
  ...
  for(id_sampling in 1:4){
    ...
    for(id_diff in 1:l){
      ...
      for(sim_id in 1:num_sim){  
        ...
        for(id_group in 1:length(setting)){
          ...
          if(id_sampling == 1){
            if(id_group == 1){
              sample <- rnorm(mean = 3, sd = 1, n = n_k)
            } else if(id_group == 2){
              sample <- rgamma (n = n_k, 
                                shape = scenario["shape"][1,] + diff.mean[id_diff], 
            ...
            if(id_group == 1){
              sample <- rnorm(mean = 3, sd = 1, n = n_k)
            } else if(id_group == 2){ #shift one group
              sample <- rgamma (n = n_k, 
                                shape = scenario["shape"][2,] + diff.mean[id_diff], 
            ...
            if(id_group == 1){
              ...
              sample <- rnorm(mean = 3 + diff.mean[id_diff], sd = 1, n = n_k)
            } else{
              sample <- rnorm(mean = 3, sd = 1, n = n_k)
            }
    ...
    picture_name.omnibus <- paste("Omnibus test ",print_setting(setting)," ",
                                  ifelse(id_sampling == 1, "[Normal - Erlang] N(3,1) vs Erl(3,1)",
                                         ifelse(id_sampling == 2, "[Normal - Erlang] N(3,1) vs Erl(15,5)",
                                                ifelse(id_sampling == 3, "[Erlang - Normal] Erl(3,1) vs N(3,1)",
                                                       ifelse(id_sampling == 4, "[Erlang - Normal] Erl(15,5) vs N(3,1)")))),
                                  ".png",sep = "")
